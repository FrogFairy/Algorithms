...
void merge(std::vector<std::pair<int, int>>& vec, std::vector<int>& count, int beg_left, int beg_right, int end_right)
{
    ...
    while (i < beg_right && j < end_right)
    {
        ...
        {
            count[vec[i].second] += (end_right - j);
...
void solution(std::vector<std::pair<int, int>>& vec, std::vector<int>& count, int beg, int end)
{
    ...
    int mid = (beg + end) / 2;
    solution(vec, count, beg, mid);
    solution(vec, count, mid, end);
    merge(vec, count, beg, mid, end);
}

std::vector<int> count_inversions(std::vector<int>& vec) {
    int n = vec.size();
    std::vector<std::pair<int, int>> nums;
    std::vector<int> counts(n, 0);
    
    for (int i = 0; i < n; ++i) {
        std::pair<int, int> pair {vec[i], i};
        nums.push_back(pair);
    }
